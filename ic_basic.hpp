...
void generateIC_basic(metadata & sim, icsettings & ic, cosmology & cosmo, mg_cosmology & quintessence, const double fourpiG, Particles<part_simple,part_simple_info,part_simple_dataType> * pcls_cdm, Particles<part_simple,part_simple_info,part_simple_dataType> * pcls_b, Particles<part_simple,part_simple_info,part_simple_dataType> * pcls_ncdm, double * maxvel, Field<Real> * pi, Field<Real> * V_pi, Field<Real> * phi, Field<Real> * chi, Field<Real> * Bi, Field<Real> * source, Field<Real> * Sij, Field<Cplx> * scalarFT_pi, Field<Cplx> * scalarFT_V_pi, Field<Cplx> * scalarFT, Field<Cplx> * BiFT, Field<Cplx> * SijFT, PlanFFT<Cplx> * plan_pi,  PlanFFT<Cplx> * plan_V_pi, PlanFFT<Cplx> * plan_phi, PlanFFT<Cplx> * plan_chi, PlanFFT<Cplx> * plan_Bi, PlanFFT<Cplx> * plan_source, PlanFFT<Cplx> * plan_Sij, parameter * params, int & numparam){
	...
	else					// initial displacements and velocities are set by individual transfer functions
	{
		...
		if (ic.tkfile[0] == '\0')
		{
			...
			loadTransferFunctions(class_background, class_perturbs, quintessence, tk_d1, tk_t1, "tot", sim.boxsize, sim.z_in, cosmo.h);
		}
		else
#endif
...
//////////////////////////////////////////////////////
 ...
 for (i = 0; i < npts; i++)
 {
   ...
   scalar_field_prime[i] = - M_PI * (tk_t_mg->y[i]) * sqrt( Pk_primordial(tk_d_mg->x[i] * cosmo.h / sim.boxsize, ic)/ tk_d_mg->x[i])/ tk_d_mg->x[i];
   // - M_PI * (tk_t_mg->y[i] * mg_field_prime/a + delta_phi * (-Hconf_quintessence  + mg_field_prime_prime/mg_field_prime )) * sqrt( Pk_primordial(tk_t_mg->x[i] * cosmo.h / sim.boxsize, ic)/ tk_t_mg->x[i])/ tk_t_mg->x[i];
 }
 // Field realization
 gsl_spline_free(tk_d_mg);
 tk_d_mg = gsl_spline_alloc(gsl_interp_cspline, npts);
 gsl_spline_init(tk_d_mg, k_mg, scalar_field, npts);
 generateRealization(*scalarFT_pi, 0., tk_d_mg, (unsigned int) ic.seed, ic.flags & ICFLAG_KSPHERE,1);
 plan_pi->execute(FFT_BACKWARD);
 pi->updateHalo();	// pi_k now is realized in real space
 gsl_spline_free(tk_d_mg);
 free(scalar_field);
 // Field derivative realization mg_prime
 gsl_spline_free(tk_t_mg);
 tk_t_mg = gsl_spline_alloc(gsl_interp_cspline, npts);
 gsl_spline_init(tk_t_mg, k_mg, scalar_field_prime, npts);
 generateRealization(*scalarFT_V_pi, 0., tk_t_mg, (unsigned int) ic.seed, ic.flags & ICFLAG_KSPHERE,1);
 plan_V_pi->execute(FFT_BACKWARD);
 V_pi->updateHalo();	// V_pi now is realized in real space
 gsl_spline_free(tk_t_mg);
 free(k_mg);
 ...
 //////////////////////////////////////////////////////
		...
		if (ic.tkfile[0] == '\0')
		{
			if (sim.gr_flag == 0)
			{
				loadTransferFunctions(class_background, class_perturbs, quintessence, tk_d1, tk_t1, NULL, sim.boxsize, sim.z_in, cosmo.h);
				...
				loadTransferFunctions(class_background, class_perturbs, quintessence, tk_d1, tk_t1, NULL, sim.boxsize, (sim.z_in + 0.01) / 0.99, cosmo.h);
				...
				loadTransferFunctions(class_background, class_perturbs, quintessence, tk_d1, tk_t1, "tot", sim.boxsize, (sim.z_in + 0.01) / 0.99, cosmo.h);

				for (i = 0; i < tk_d1->size; i++) // construct gauge correction for N-body gauge velocities
					temp1[i] = -99.5 * gsl_spline_eval(quintessence.spline_H, 0.995 * a,quintessence.acc_H) * (3. * temp1[i] * M_PI * sqrt(Pk_primordial(tk_d1->x[i] * cosmo.h / sim.boxsize, ic) / tk_d1->x[i]) / tk_d1->x[i] + (temp2[i] + 3. * gsl_spline_eval(quintessence.spline_H, 0.99 * a,quintessence.acc_H) * M_PI * tk_t1->y[i] * sqrt(Pk_primordial(tk_d1->x[i] * cosmo.h / sim.boxsize, ic) / tk_d1->x[i]) / tk_d1->x[i] / tk_d1->x[i] / tk_d1->x[i]));
			...
			loadTransferFunctions(class_background, class_perturbs, quintessence, tk_d1, tk_t1, "cdm", sim.boxsize, sim.z_in, cosmo.h);
		}
		else
#endif
...
#ifdef HAVE_CLASS
			if (ic.tkfile[0] == '\0')
				loadTransferFunctions(class_background, class_perturbs, quintessence, tk_d2, tk_t2, "b", sim.boxsize, sim.z_in, cosmo.h);
			else
#endif
	...
	for (p = 0; p < cosmo.num_ncdm; p++)	// initialization of non-CDM species
	{
		...
		if (ic.pkfile[0] == '\0')
		{
			...
			if (ic.tkfile[0] == '\0')
      {
				loadTransferFunctions(class_background, class_perturbs, quintessence, tk_d1, tk_t1, ncdm_name, sim.boxsize, sim.z_in, cosmo.h);
      }
			else
#endif
