...
//   pi             pointer to allocated field
//   V_pi           pointer to allocated field
...
#else
    if (sim.downgrade_factor > 1)
      pi->saveHDF5_coarseGrain3D(h5filename + filename + "_pi.h5", sim.downgrade_factor);
    else
      pi->saveHDF5(h5filename + filename + "_pi.h5");
#endif

if (sim.out_snapshot & MASK_V_PI)
#ifdef EXTERNAL_IO
  V_pi->saveHDF5_server_write(NUMBER_OF_IO_FILES);
#else
  if (sim.downgrade_factor > 1)
	...
	if (sim.out_snapshot & MASK_GADGET)
	{
		...
		dtau_pos = (hdr.time - a) / a / Hconf_quintessence;
#else
		hdr.time = a;
		hdr.redshift = (1./a) - 1.;
#endif
...
void writeLightcones(metadata & sim, cosmology & cosmo, mg_cosmology & quintessence, const double fourpiG, const double a, const double tau, const double dtau, const double dtau_old, const double maxvel, const int cycle, string h5filename, Particles_gevolution<part_simple,part_simple_info,part_simple_dataType> * pcls_cdm, Particles_gevolution<part_simple,part_simple_info,part_simple_dataType> * pcls_b, Particles_gevolution<part_simple,part_simple_info,part_simple_dataType> * pcls_ncdm, Field<Real> * phi, Field<Real> * chi, Field<Real> * Bi, Field<Real> * Sij, Field<Cplx> * BiFT, Field<Cplx> * SijFT, PlanFFT<Cplx> * plan_Bi, PlanFFT<Cplx> * plan_Sij, int & done_hij, set<long> * IDbacklog)
{
	...
	for (i = 0; i < sim.num_lightcone; i++)
	{
		...
		d = gsl_spline_eval(quintessence.spline_particleHorizon, 1. / (1. + sim.lightcone[i].z), quintessence.acc_particleHorizon);
		...
		if (sim.out_lightcone[i] & MASK_GADGET && sim.lightcone[i].distance[0] > d - tau + 0.5 * dtau_old && sim.lightcone[i].distance[1] <= d - tau + 0.5 * dtau_old && d - tau + 0.5 * dtau_old > 0.)
		{
			...
			if (sim.tracer_factor[0] > 0)
				pcls_cdm->saveGadget2(h5filename + filename + "_cdm", hdr, sim.lightcone[i], d - tau, dtau, dtau_old, a * Hconf_quintessence, vertex, n, IDbacklog[0], IDprelog[0], phi, sim.tracer_factor[0]);

			if (sim.baryon_flag && sim.tracer_factor[1] > 0)
			{
				hdr.mass[1] = (double) sim.tracer_factor[1] * C_RHO_CRIT * cosmo.Omega_b * sim.boxsize * sim.boxsize * sim.boxsize / sim.numpcl[1] / GADGET_MASS_CONVERSION;
				pcls_b->saveGadget2(h5filename + filename + "_b", hdr, sim.lightcone[i], d - tau, dtau, dtau_old, a * Hconf_quintessence, vertex, n, IDbacklog[1], IDprelog[1], phi, sim.tracer_factor[1]);
			}
			...
			for (p = 0; p < cosmo.num_ncdm; p++)
			{
				...
				pcls_ncdm[p].saveGadget2(h5filename + filename + buffer, hdr, sim.lightcone[i], d - tau, dtau, dtau_old, a * Hconf_quintessence, vertex, n, IDbacklog[p+1+sim.baryon_flag], IDprelog[p+1+sim.baryon_flag], phi, sim.tracer_factor[p+1+sim.baryon_flag]);
			}
		}
	}
...
)
{
	...
	double Hconf_quintessence = gsl_spline_eval(quintessence.spline_H,a,quintessence.acc_H);
  double Hconf0_quintessence = gsl_spline_eval(quintessence.spline_H,1.,quintessence.acc_H);
  ...
  //qintessence
   if (sim.out_pk & MASK_PI)
   {
     ...
     writePowerSpectrum(kbin, power, kscatter, pscatter, occupation, sim.numbins, sim.boxsize, (Real) numpts3d * (Real) numpts3d * 2. * M_PI * M_PI / Hconf0_quintessence / Hconf0_quintessence, filename, "power spectrum of mg field * H0  (dimension less)", a, sim.z_pk[pkcount]);
   }
   ...
   if (sim.out_pk & MASK_V_PI)
   {
     ...
     writePowerSpectrum(kbin, power, kscatter, pscatter, occupation, sim.numbins, sim.boxsize, (Real) numpts3d * (Real) numpts3d * 2. * M_PI * M_PI, filename, "power spectrum of mg_prime (dimensionless)", a, sim.z_pk[pkcount]);
   }
