...
double mg_pot(const double mg_field_bk, const double mg_pot_const, const double mg_pot_exp) {

  return mg_pot_const * pow(mg_field_bk, - mg_pot_exp);

}

double mg_pot_p(const double mg_field_bk, const double mg_pot_const, const double mg_pot_exp) {

  return - mg_pot_exp * mg_pot_const * pow(mg_field_bk, - mg_pot_exp - 1.);

}

double mg_coupl(const double mg_field_bk, const double mg_coupl_const) {
...
double mg_coupl_p(const double mg_field_bk ,const double mg_coupl_const) {

  return 2.*mg_coupl_const * mg_field_bk;

}

double mg_coupl_pp(const double mg_field_bk, const double mg_coupl_const) {

  return 2.*mg_coupl_const;

}

double mg_field_bk_pp_rk4(const double a, const double fourpiG, const cosmology cosmo, const double a_p, const double mg_field_bk, const double mg_field_p_bk, const double mg_coupl_const, const double mg_pot_const, const double mg_pot_exp )
{
  double a2 = a * a;
  double rhoa2 = (cosmo.Omega_cdm + cosmo.Omega_b) / a + (cosmo.Omega_Lambda * a2) + (cosmo.Omega_rad / a / a);
  double Pa2 = - (cosmo.Omega_Lambda * a2) + 1./3. * (cosmo.Omega_rad / a / a );

	return - 2. * a_p/a * mg_field_p_bk + ( - mg_pot_p(mg_field_bk, mg_pot_const, mg_pot_exp) * a2 * ( 1. + mg_coupl(mg_field_bk, mg_coupl_const)) + ( rhoa2 - 3. * Pa2 + 4. * a2 * mg_pot(mg_field_bk, mg_pot_const, mg_pot_exp) - mg_field_p_bk * mg_field_p_bk * ( 1. + 3./2./fourpiG * mg_coupl_pp(mg_field_bk, mg_coupl_const) ) ) /2. * mg_coupl_p(mg_field_bk, mg_coupl_const) ) / (1. + mg_coupl(mg_field_bk, mg_coupl_const) + 3./4./fourpiG * mg_coupl_p(mg_field_bk, mg_coupl_const) * mg_coupl_p(mg_field_bk, mg_coupl_const) );
}
#endif
