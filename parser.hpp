		...
		if (strcmp(params[i].name, pname) == 0)
		{
			...
			while ((comma = strchr(start, ',')) != NULL)
			{
				...
				if (strcmp(item, "Phi") == 0 || strcmp(item, "phi") == 0)
					pvalue |= MASK_PHI;
        else if (strcmp(item, "Pi") == 0 || strcmp(item, "pi") == 0)
          pvalue |= MASK_PI;
        else if (strcmp(item, "V_Pi") == 0 || strcmp(item, "V_pi") == 0)
          pvalue |= MASK_V_PI;
			...
			if (strcmp(start, "Phi") == 0 || strcmp(start, "phi") == 0)
				pvalue |= MASK_PHI;
      else if (strcmp(start, "Pi") == 0 || strcmp(start, "pi") == 0)
        pvalue |= MASK_PI;
      else if (strcmp(start, "V_Pi") == 0 || strcmp(start, "V_pi") == 0)
        pvalue |= MASK_V_PI;
...
int parseMetadata(parameter * & params, const int numparam, metadata & sim, cosmology & cosmo, mg_cosmology & quintessence, icsettings & ic)
{
  ...
  // Quintessence
	...
	// Parse MG parameters
	if (!parseParameter(params, numparam, "quintessence background file", quintessence.mg_bkg_file))
		strcpy(quintessence.mg_bkg_file, "");

	if (!parseParameter(params, numparam, "quintessence verbose", quintessence.mg_verbose))
		quintessence.mg_verbose = 0;

	// Parse action parameters
	if (!parseParameter(params, numparam, "quintessence alpha", quintessence.mg_alpha))
		quintessence.mg_alpha = 0.;
	if (!parseParameter(params, numparam, "quintessence Lambda", quintessence.mg_Lambda))
  ...
  if (!parseParameter(params, numparam, "quintessence ic_phi_ini", quintessence.ic_phi))
